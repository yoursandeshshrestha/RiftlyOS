...
export function TaskBoard({
  ...
  if (isLoading) {
    return (
      <>
        {[...Array(4)].map((_, i) => (
          <div key={i} className="flex w-[300px] shrink-0 flex-col rounded-md bg-board-column p-3 sm:w-[380px]">
            {/* Column Header */}
            ...
            <div className="flex min-h-[160px] flex-col gap-2">
              {[...Array(2)].map((_, j) => (
                <div key={j} className="surface-card flex min-h-[140px] cursor-pointer flex-col space-y-2 rounded-md p-3">
                  {/* Task title */}
